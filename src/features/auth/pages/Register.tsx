      ...
      <main className="flex-1 flex items-center justify-center px-6 lg:px-12 py-12">
        ...
        >
          <div className="lg:w-1/2 bg-secondary p-10 lg:p-14 text-white flex flex-col justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <p className="text-white/80 text-lg leading-relaxed">
                Créez votre compte en moins de 2 minutes. Accédez à des campagnes SMS, Email et WhatsApp avec une précision chirurgicale.
              </p>
              <ul className="space-y-3 pt-4">
                {['14 jours d\'essai gratuit', 'Aucune carte bancaire requise', 'Support dédié 24/7'].map((item, i) => (
                  <li key={i} className="flex items-center gap-3 text-sm font-semibold">
                    <div className="w-5 h-5 rounded-full bg-primary/30 flex items-center justify-center">✓</div>
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="lg:w-1/2 p-8 lg:p-12 bg-white">
            <div className="mb-8">
              <h2 className="font-headline text-2xl font-bold text-secondary mb-2">Créer un compte expert</h2>
              <p className="text-on-surface-variant text-sm">Remplissez les informations ci-dessous pour initialiser votre espace marchand.</p>
            </div>
            <RegisterForm />
            <div className="mt-8 text-center">
              <Link to="/login" className="inline-flex items-center gap-2 text-sm font-semibold text-secondary/70 hover:text-primary transition-colors">
