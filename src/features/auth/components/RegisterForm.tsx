...
export default function RegisterForm() {
  ...
  const initialEmail = (location.state as { email?: string } | null)?.email ?? localStorage.getItem('novasms-pending-confirmation-email') ?? '';

  const { register, handleSubmit, formState: { errors }, watch, setValue } = useForm<RegisterDto>({
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input {...register('nom')} className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Ex: Konan Romuald" />
        ...
        <div>
          ...
          <CountrySelect value={watch('pays')} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setValue('pays', e.target.value)} error={errors.pays?.message} />
        </div>
      </div>
      
      <div>
        <label className="block text-sm font-semibold text-on-surface mb-1">Email professionnel</label>
        <input {...register('email')} type="email" className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="contact@boutique.ci" />
        {errors.email && <p className="mt-1 text-xs text-red-600">{errors.email.message}</p>}
      </div>
      
      <div>
        <label className="block text-sm font-semibold text-on-surface mb-1">Nom de la boutique</label>
        <input {...register('nomBoutique')} className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Ex: NovaShop CI" />
        {errors.nomBoutique && <p className="mt-1 text-xs text-red-600">{errors.nomBoutique.message}</p>}
      </div>
      
      <div>
        <label className="block text-sm font-semibold text-on-surface mb-1">Mot de passe</label>
        <div className="relative">
          <input {...register('motDePasse')} type={showPassword ? 'text' : 'password'} className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface focus:outline-none focus:ring-2 focus:ring-primary/50 pr-10" placeholder="••••••••" />
      ...
      <div className="flex items-start gap-3 pt-2">
        ...
        <label htmlFor="cgu" className="text-sm text-on-surface-variant">
          J'accepte les <a href="#" className="text-primary">Conditions</a> et la <a href="#" className="text-primary">Politique de confidentialité</a>
        </label>
      </div>
      
      <button type="submit" disabled={isLoading} className="w-full bg-primary text-white font-bold py-4 rounded-xl hover:brightness-110 flex items-center justify-center gap-2 disabled:opacity-70">
