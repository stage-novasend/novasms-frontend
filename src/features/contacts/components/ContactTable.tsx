...
export default function ContactTable({ onContactClick, onImportClick }: ContactTableProps) {
  ...
  const createSegmentFromSelection = async () => {
    if (selectedContactIds.size === 0) return;

    const name = segmentName.trim() || `Segment sélection (${selectedContactIds.size})`;
    setSavingSegment(true);
    setSegmentError(null);
    try {
      const result = await contactsApi.createSegmentFromContacts(
        name,
      ...
      const message =
        typeof error === 'object' && error !== null && 'response' in error
          ...
          : error instanceof Error
            ? error.message
            : null;
      setSegmentError(message || 'Impossible de créer le segment depuis la sélection');
    } finally {
      setSavingSegment(false);
    }
  };
      ...
      <div className="text-center py-12">
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            ...
            data-tour="contacts-import-button"
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {selectedContactIds.size > 0 && (
            <button
              ...
              data-tour="contacts-create-segment-button"
              disabled={isDeleting || savingSegment}
              className="px-4 py-2 border border-primary text-primary font-medium rounded-lg hover:bg-primary/5 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Plus className="w-4 h-4" />
              Créer segment ({selectedContactIds.size})
            </button>
          )}
          {selectedContactIds.size > 0 && (
            <button
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          data-tour="contacts-new-segment-button"
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                data-tour="contacts-save-segment-button"
          ...
          <tbody className="divide-y divide-outline-variant/50">
            {contacts.map((contact, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 cursor-pointer" onClick={() => onContactClick?.(contact)}>
                  <div className="flex items-center gap-3">
