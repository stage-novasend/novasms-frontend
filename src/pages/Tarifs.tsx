  ...
  const plans = [
    {
      ...
      features: ["14 jours d'essai gratuit", "500 crédits offerts", "1 canal (SMS)", "Support communauté", "Analytics basiques"],
    ...
    },
    {
      ...
      features: ["5 000 crédits inclus", "SMS + Email + WhatsApp", "Automatisations avancées", "Support prioritaire", "API complète", "Analytics temps réel"],
    ...
    },
    {
      ...
      features: ["Crédits illimités", "IP dédiée & SLA 99.9%", "Account manager dédié", "Intégrations sur mesure", "Formation équipe", "Conformité RGPD avancée"],
    ...
    <div className="min-h-screen bg-background text-on-surface font-body antialiased">
      ...
      <main className="pt-32 pb-24 px-6 lg:px-12">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="text-center max-w-3xl mx-auto mb-16">
            <h1 className="font-headline text-5xl lg:text-6xl font-extrabold text-secondary tracking-tight mb-6">Des tarifs transparents, sans surprise</h1>
            <p className="text-xl text-on-surface-variant">Choisissez le plan qui correspond à votre volume. Évoluez ou downgradez à tout moment.</p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-20">
            {plans.map((plan, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                {plan.popular && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-white text-xs font-bold px-4 py-1.5 rounded-full">Le plus populaire</div>}
                <h3 className="font-headline text-2xl font-bold text-secondary mb-2">{plan.name}</h3>
                ...
                <button onClick={() => navigate('/register')} className={`w-full py-3 rounded-xl font-bold transition-all ${plan.popular ? 'bg-primary text-white hover:brightness-110' : 'bg-surface-variant text-secondary hover:bg-surface-variant/70'}`}>
                  {plan.cta}
                </button>
              </motion.div>
            ))}
          ...
          <div className="bg-secondary rounded-[32px] p-12 lg:p-20 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              <h2 className="font-headline text-4xl font-extrabold text-white">Vous hésitez encore ?</h2>
              <p className="text-white/70 text-lg max-w-2xl mx-auto">Profitez de 14 jours gratuits sur le plan Professionnel. Aucune carte requise.</p>
              <button onClick={() => navigate('/register')} className="bg-primary text-white font-bold py-5 px-12 rounded-xl text-lg hover:brightness-110 transition-all shadow-2xl shadow-primary/30">Démarrer l'essai gratuit</button>
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="py-12 px-6 lg:px-12 border-t border-outline-variant/20 bg-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-primary rounded flex items-center justify-center"><svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg></div>
